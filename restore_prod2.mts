...
// Build connection from PGHOST/PGUSER/PGPASSWORD/PGDATABASE if available
// Otherwise fallback to DATABASE_URL or NEON_DATABASE_URL
let dbUrl: string;
if (process.env.PGHOST && process.env.PGHOST !== 'helium' && process.env.PGHOST !== 'localhost') {
  const user = encodeURIComponent(process.env.PGUSER || 'neondb_owner');
  ...
  console.log("Using PGHOST connection to:", host);
} else {
  dbUrl = process.env.NEON_DATABASE_URL || process.env.DATABASE_URL || '';
  console.log("Using URL connection (first 40 chars):", dbUrl.substring(0, 40));
}

const sql = neon(dbUrl);

async function run() {
  const check = await sql`SELECT current_database() as db, COUNT(*) as total FROM daily_sales`;
  console.log(`Database: ${check[0].db}, Current rows: ${check[0].total}`);
  
  // Check if this is the right database (production should have 542 rows, not 573)
  if (parseInt(check[0].total) !== 542) {
    console.log("This doesn't appear to be the production database (expected 542 rows). Stopping.");
    return;
  }
  ...
  for (const tx of transactions) {
    ...
    await sql`
      ...
      ON CONFLICT (date, order_channel) DO UPDATE SET
        net_sales = EXCLUDED.net_sales, total_sales = EXCLUDED.total_sales,
        imported_by = ${adminId}, imported_at = NOW()
    `;
    inserted++;
  ...
  console.log(`\n✅ Done: ${inserted} records restored. Total now: ${after[0].c}`);
}
