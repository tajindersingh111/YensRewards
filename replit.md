...
**Current Version: v3.11.2** - Production deployment support: Added "Seed Default Sites" feature with one-click button to populate all 15 original sales channels (SHOP, GRAB, FOODPANDA, LINEMAN, SHOPEE, SHOPZY, SUPALAI, BALLOON, BOX, RIVER, ARMY, LAMP, CNY, UNIVERSITY, G2) in production environment. Implemented `storage.bulkCreateSites()` method with atomic transaction support to ensure all-or-nothing creation. Button appears in Sites tab when fewer than 5 sites exist, includes confirmation dialog, loading states, and comprehensive error handling. Full bilingual support (EN/TH) via i18n. Maintains storage abstraction layer for future scalability. Successfully tested end-to-end with OIDC authentication and Sales Tracker integration. Eliminates need for manual SQL or individual site creation in production. Previous version (v3.11.1): Enhanced error handling for Sales Tracker dropdown with comprehensive loading, error, and empty states.
