...
**Current Version: v3.9.1** - Bug fix: Resolved validation error when adding sales via manual form. The `dayOfWeek` field is now automatically calculated server-side from the selected date, eliminating the "Required at 'dayOfWeek'" error. Sales Tracker Dashboard features single-row layout with 6 KPI boxes where first 3 boxes (Current Week, Current Month, YTD) are larger (col-span-2) with gradient backgrounds, and last 3 boxes (Best Channel, Best Day, Best Month) are smaller (col-span-1) with flat colors. All date calculations use UTC for timezone consistency.
