...
**Current Version: v3.11.4** - **Excel Upload Bug Fix**: Fixed critical bug where Sales Tracker Excel upload feature was calling non-existent endpoint `/api/admin/sales/bulk-import` instead of correct endpoint `/api/admin/import-sales-excel`, causing all Excel uploads to fail silently. Extended `apiRequest` helper in `queryClient.ts` to support FormData uploads while maintaining shared headers/credentials and unified error handling. Removed unused XLSX import from frontend. Excel data imports now function correctly for populating Analytics Dashboard. Previous version (v3.11.3): Production-ready with critical race condition fixes in transaction processing and barista performance tracking, atomic database operations, and enforced data integrity constraints.
