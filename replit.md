...
**Current Version: v3.9.0** - Sales Tracker Dashboard enhancement: Added Current Week Revenue and Current Month Revenue KPI boxes. Implemented single-row layout with 6 KPI boxes where first 3 boxes (Current Week, Current Month, YTD) are larger (col-span-2) with gradient backgrounds, and last 3 boxes (Best Channel, Best Day, Best Month) are smaller (col-span-1) with flat colors. Compact design using grid-cols-9 for optimal space utilization. All date calculations use UTC for timezone consistency. Fixed OIDC admin role assignment bug in test mode to support automated testing.
