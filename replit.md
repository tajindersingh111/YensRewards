...
**Current Version: v3.11.2** - Production deployment support: Added "Seed Default Sites" feature with one-click button to populate all 15 original sales channels (SHOP, GRAB, FOODPANDA, LINEMAN, SHOPEE, SHOPZY, SUPALAI, BALLOON, BOX, RIVER, ARMY, LAMP, CNY, UNIVERSITY, G2) in production environment. Button appears in Sites tab when fewer than 5 sites exist, includes confirmation dialog, and handles loading/error states. Eliminates need for manual SQL or individual site creation. Previous version (v3.11.1): Enhanced error handling for Sales Tracker dropdown with comprehensive loading, error, and empty states.
