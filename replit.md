...
**Current Version: v3.17.57** - **Automations + Mass-send improvements**: Full Automations UI (filter bar, quickstart templates, template picker, per-card test-send with channel-aware recipient, run history panel); backend test-send route; mass-send live progress bar with `{sent}/{total}` polling every 3s; LINE sends now hook into job polling; send report dialog shown on completion.
