...
**Current Version: v3.13.1** - **Data Downloads Center**: Added dedicated Data Downloads section in Settings for easy admin access to export files. Includes secure admin-only download endpoint (`/api/admin/downloads/`), professional UI with file cards and one-click downloads, and persistent storage for failed customer records (133 customers from backup CSV). Future-proof design allows easy addition of more export files. Previous version (v3.13.0): CSV Import Protection & Error Messages - Fixed critical bug overwriting customer names, enhanced error messages with context-aware guidance for invalid points values (detects dates/times), and improved validation distinguishing between date-like values, fractional numbers, and invalid text.
