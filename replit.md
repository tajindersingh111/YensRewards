...
**Current Version: v3.11.0** - Sites-to-Sales Channel Integration: Sales channels are now managed through the Sites system, eliminating hardcoded channel arrays. Each site has a required `channelName` field (1-20 uppercase chars: letters, numbers, spaces, hyphens, underscores) displayed in yellow badges. Sales Tracker dropdown dynamically populates from active sites, automatically updating when sites are added/edited/disabled. Comprehensive validation enforces data integrity at schema (Zod with `/^[A-Z0-9 _-]+$/` regex), backend (POST/PATCH routes using insertSiteSchema.partial()), and frontend (client filtering) levels. Full bilingual support for channel management UI (EN/TH). E2E tested and verified. Previous version (v3.10.0): Sales record editing and deletion with CRUD functionality.
