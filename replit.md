...
**Current Version: v3.19.0** - **Barista Management tab**: New Operations → Barista section with Overview (per-barista weekly summary with scheduled vs actual hours, attendance %), Attendance log (all clock-in/out records filterable by barista), Hours Summary (date-range aggregation with CSV export), and Performance (weekly metrics: transactions, new signups, specials sold, points, rank): Full Automations UI (filter bar, quickstart templates, template picker, per-card test-send with channel-aware recipient, run history panel); backend test-send route; mass-send live progress bar with `{sent}/{total}` polling every 3s; LINE sends now hook into job polling; send report dialog shown on completion.
...
- **Admin Navigation:** Two-level navigation system. Primary nav: Dashboard, Sales, Customers, Marketing, Operations, Admin. Secondary sub-nav per section: Dashboard→Overview; Sales→Sales Tracker/Analytics; Customers→Customer List/Loyalty; Marketing→Messages/Promotions/Weekly Specials; Operations→Products/Sites/Schedules/Barista; Admin→Users/Settings. Full-bleed rendering for Sales Tracker and Analytics tabs. URL routing via `?section=&tab=` query params with backward compat for old `?tab=` format.
...
- **Marketing Automations:** Rule-based automated messaging in Marketing→Automations tab. Features: trigger types (recurring daily/weekly/monthly, one-time), audience filters (all, tier, birthday today, inactive 30d/60d), channel selection (app/LINE/SMS/email), email subject support, enable/disable toggle, run history log per automation (with sent/failed counts and timestamps), delete with confirmation. Automations fire via the background scheduler (every 60s checks for due automations based on `nextRunAt`). Bangkok timezone (UTC+7) for all schedule calculations. Post-fire: nextRunAt recalculated for recurring; one-time automations auto-disable after firing.
