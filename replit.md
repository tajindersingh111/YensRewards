...
**Current Version: v3.11.1** - Enhanced error handling for Sales Tracker dropdown: Added comprehensive loading, error, and empty states to prevent silent failures when sites API is unavailable. Dropdown now displays clear messages for loading, API errors, or no active sites, ensuring better user experience and troubleshooting. Previous version (v3.11.0): Sites-to-Sales Channel Integration with dynamic channel management through Sites system. Full bilingual support (EN/TH), comprehensive validation at all levels, E2E tested.
