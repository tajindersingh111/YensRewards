...
**Current Version: v3.13.0** - **CSV Import Protection & Error Messages**: Fixed critical CSV import bug that was overwriting existing customer names with incorrect data. CSV imports now intelligently preserve existing customer names while updating points, tier, and other fields. Only fills in missing names (empty/blank) from CSV data. Enhanced error messages with context-aware guidance: detects when date/time values appear in points column and provides actionable advice about column mapping issues. Includes improved points validation that distinguishes between date-like values, fractional numbers, and invalid text. Previous version (v3.12.1): Streamlined Messages & LINE Integration - Final streamlined Messages interface with 2 main tabs (Send/History) and 4 channel tabs (App/LINE/SMS/Email). LINE messaging fully integrated with customer filtering, tier targeting, 5,000 character limit, FREE messaging, and dedicated API.
