...
**Current Version: v3.12.1** - **Streamlined Messages & LINE Integration**: Final streamlined Messages interface with 2 main tabs (Send/History) and 4 channel tabs (App/LINE/SMS/Email). LINE messaging now fully integrated into channel selection with complete functionality: customer filtering by LINE connection status, tier-based or individual targeting, 5,000 character limit, FREE messaging, benefits education, and dedicated /api/admin/messages/send-line API. Removed "View This Morning's Version" legacy button. Confirmed CSV import duplicate handling via upsert (updates existing customers by phone instead of creating duplicates). Previous version (v3.11.9): Streamlined Messages Interface - Reorganized Messages tabs from 4 to 3 tabs (Send/History/LINE) by removing Templates tab and moving LINE to 3rd position for easier access. Maintains beautiful Yens yellow theme with alternating colors.
