...
export default function PointsCard({ points, tier, nextTierPoints }: PointsCardProps) {
  ...
  return (
    <Card className="p-6 bg-primary text-primary-foreground" data-testid="points-card">
      <div className="flex flex-col items-center gap-4">
        <Badge className={`${tierColors[tier]} px-4 py-1`} data-testid={`badge-tier-${tier}`}>
          {tierNames[tier]} Member
        </Badge>
        
        <div className="text-center">
          <p className="text-lg font-medium opacity-90">Your Points</p>
          <p className="text-6xl font-bold" data-testid="text-points">{points}</p>
        </div>
